...
//view_skin.php
function caView($val,$type,$gubun='') {
	GLOBAL $view,$board;
...
<!-------예약 항목 끝------------------------>
		...
		<tr>
			<th scope="row"><label for="wr_8">접수상황</label></th>
			<td>
			
			<? if ($write[wr_8] == "2"){ 
			echo "예약 처리되었습니다. ^^"; } 
			else echo "예약 대기중 입니다. ^^"?>
		...
		<tr>
			...
			<td>			
<? if ($is_admin) { echo "<a href=\"javascript:win_open('$board_skin_url/pending.php?bo_table=$bo_table&wr_id=$wr_id', 'FIELDUP', target='hiddenframe');\" class='btn_b01'>예약대기</a> "; } ?>
<? if ($is_admin) { echo "<a href=\"javascript:win_open('$board_skin_url/complete.php?bo_table=$bo_table&wr_id=$wr_id', 'FIELDUP', target='hiddenframe');\" class='btn_b01'>예약완료</a> "; } ?>			
