...
function get_list_thumbnail2($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=true, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    ...
    $tname = thumbnail2($filename, $filepath, $filepath, $thumb_width, $thumb_height, $is_create, $is_crop, $crop_mode, $is_sharpen, $um_value);
...
function get_view_thumbnail2($contents, $thumb_width=0)
{
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            $is_animated = false;
            if($size[2] == 1) {
                $is_animated = is_animated_gif2($srcfile);
            }
            ...
            // 썸네일 생성
            if(!$is_animated)
                $thumb_file = thumbnail2($filename, $filepath, $filepath, $thumb_width, $thumb_height, false);
            else
                $thumb_file = $filename;
...
function thumbnail2($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    global $g5;
    ...
    if($size[2] == 1) {
        if(is_animated_gif2($source_file))
    ...
    if($is_sharpen && $is_large) {
        ...
        UnsharpMask2($dst, $val[0], $val[1], $val[2]);
    }
...
function UnsharpMask2($img, $amount, $radius, $threshold)    {
...
function is_animated_gif2($filename) {
